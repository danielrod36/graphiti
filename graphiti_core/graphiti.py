...
class Graphiti:
    def __init__(
        ...
        cross_encoder : CrossEncoderClient | None, optional
            ...
            If not provided, a NoopReranker will be used (no reranking).
    ...
    async def search_with_expansion(
        ...
        if self.cross_encoder and merged_edges:
            reranked = await self.cross_encoder.rank(query, [e.fact for e in merged_edges])
            # Map back to edges in reranked order
            fact_to_edge = {e.fact: e for e in merged_edges}
            merged_edges = [fact_to_edge[fact] for fact, _ in reranked if fact in fact_to_edge]
